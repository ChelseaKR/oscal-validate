...
| Default path byte identity | The default command's stdout and exit code over the fixtures and nine published documents equal the goldens first captured from commit `6978895`, the last before ADR-0005, and recaptured five times since from the same documents, most recently on 2026-09-07 (#88), each reason recorded in the docstring of the test module named here; a validation run in a fresh process loads neither `oscal_validate.ai` nor the SDK | `tests/test_default_path_byte_identity.py` | AUTO | Maintainer |
...
| Spec snapshot freshness | Re-vendor and re-hash on a new OSCAL release, with its cost read before anything is written | Noticing a release is still a manual check against the OSCAL releases page. `tools/revendor.py <version>` then prints what it would change -- constraints, the parsed grammar, schema definitions, the bytes of every vendored file, and each golden that would move -- and writes only with `--write` (`tests/test_revendor.py`) | REVIEW | Maintainer |
