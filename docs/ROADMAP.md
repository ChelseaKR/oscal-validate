...
| Default path byte identity | The default command's stdout and exit code over the fixtures and nine published documents equal the goldens first captured from commit `6978895`, the last before ADR-0005, and recaptured six times since from the same documents, most recently on 2026-09-07 (#77), each reason recorded in the docstring of the test module named here; a validation run in a fresh process loads neither `oscal_validate.ai` nor the SDK | `tests/test_default_path_byte_identity.py` | AUTO | Maintainer |
